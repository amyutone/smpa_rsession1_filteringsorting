...
impeach %>% filter(for_impeachment == "YES", date_month == 9)
...
impeach %>% arrange(clinton_percent >= 70, )
...
#impeach %>% filter(for_impeachment == "NO", pct_bachelors clinton_percent >= 70)
...
qseven[order(qseven$pct_bachelors), ]
...
qeight <- impeach %>% 
  select(for_impeachment,pct_bachelors_compared_to_national) %>% 
  filter(for_impeachment == "NO", pct_bachelors_compared_to_national == "BELOW") 
qeight
  
qeight[order(qeight$pct_bachelors_compared_to_national), ]
...
```{r}
qten <- impeach %>% 
  select(for_impeachment,date_year, clinton_percent) %>% 
  filter(for_impeachment == "YES", date_year<2019) %>% 
  arrange(desc(clinton_percent))
qten

#all columns except: party...DH<- impeach %>% 
  #select(-party,-state,-for_impeachment)
#DH
